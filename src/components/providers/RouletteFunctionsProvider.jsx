...
export function RouletteFunctionsProvider({children}) {
  ...
  const [shootAnimation, setShootAnimation]=useState(false); //activa la animacion del efecto disparo
  const [damageAnimationPlayer1,setDamageAnimationPlayer1]=useState(false); //animacion de daño
  const [damageAnimationPlayer2,setDamageAnimationPlayer2]=useState(false); //animacion de daño
  ...
  function HandleButtonsFire()  { // Cambia el estado para mostrar u ocultar los botones de disparo al hacer click en la escopeta
    ...
    function ToggleBulletsVisibility() { //Muestra las balas  en el visualizador de balas 
     
        setShowBoxBulletsClass('invertY');  // Establece la clase 'invertY' después de 1 segundo
      ...
      function Renderbullets(arraybullets = []) { //renderizado de balas
        let bulletsRender = [];
        let redBullets = [];
        let blueBullets = [];
        arraybullets.forEach(bullet => { // Itera sobre el array de balas y separa las rojas y azules
          if (bullet === 'red') {
        ...
        bulletsRender = blueBullets.concat(redBullets);   // Concatena primero las balas rojas y luego las azules
    ...
    if (shooter === target) { // El jugador se dispara a sí mismo
      if (bullet === 'red') { // Si la bala es roja, el jugador recibe daño y cambia de turno
        ...
        target === 'player1' ? ActionAndAudio(setPlayer1Health(prevHealth => prevHealth - 1),'Escopeta/shoot',1,'Escopeta/saltabala_masrecarga') : ActionAndAudio(setPlayer2Health(prevHealth => prevHealth - 1),'Escopeta/shoot',1,'Escopeta/saltabala_masrecarga');
          ...
          if(target==='player1')setDamageAnimationPlayer1(true);
          if(target==='player2')setDamageAnimationPlayer2(true);

          setTimeout(function(){
            setDamageAnimationPlayer1(false);
            setDamageAnimationPlayer2(false);
          },800);
  ...
  function ActionAndAudio(functionAction, audiomp3, repeticionsonido = 1, audiomp3two){
    ...
    let contador = 0;
    const reproducirAudio = () => { // Función para reproducir el audio
    ...
    function CuatroConsecutivosIguales(array) { // Verifica que no alla 4 balas seguidas del mismo color en la recamara
      ...
      for (var i = 0; i < array.length - 3; i++) {
          ...
          if (array[i] === array[i + 1] &&
              array[i] === array[i + 2] &&
              array[i] === array[i + 3]) { 
              return true;   // Si encontramos cuatro elementos iguales consecutivos, devolvemos true
          }
      }
      return false; // Si no encontramos cuatro elementos iguales consecutivos, devolvemos false
    };
    ...
    function Loadchamber() { //carga de camara de escopeta con balas.
      ...
      let newBullets;
      do {
          redBullets = Math.floor(Math.random() * balasRandomTotal); // Genera un número aleatorio de balas rojas
          blueBullets = balasRandomTotal - redBullets; // Calcula el número de balas azules
          newBullets = Array(balasRandomTotal).fill('blue'); // Llena el array con balas azules inicialmente
          for (let i = 0; i < redBullets; i++) {  // Cambia el color de las balas correspondientes a rojo
...
//---------------------ITEMS---------------------
  ...
  function VerBala(){ //muestra bala en recamara del arma
  ...
  function SaltarBala() { //quita la bala en la recamara para preparar la siguente. 
    const newBullets = [...bullets];    // Copia del array de balas
    ...
    // Actualiza el estado de la bala actual en recamara
...
//_--------------------------HERRAMIENTAS----------------------------------
    ...
    function ReloadButton() {// Recargar la página
    
      window.location.reload(); 
    };

    function GenerarNumeroRandom(min, max) {  // Genera un número aleatorio entre min y max (incluyendo ambos)
    ...
    function ContarRepeticiones(arr, elemento) { //cuenta la cantidad de veces que se repite un elemento dentro de un array
      let contador = 0;
    ...
    function GenerarArrayDeKeys(array) {    //Devuelve un array de key con la cantidad repetida de cada key acorde a lo solicitado.
